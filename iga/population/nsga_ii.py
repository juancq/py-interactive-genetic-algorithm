...
#---------------------------------------#
    def nextgen(self):
        ...
        '''
        newPop = []
        fronts = self.createFronts(self.pop)
        self.crowdedDistance(fronts)
        random = self.params.random

        for i in xrange(0, self.popsize, 2):
            p1 = self.selectfunc(fronts, random)
            p2 = self.selectfunc(fronts, random)
        ...
        r = copy.deepcopy(self.pop)
        r.extend(newPop)
        new_fronts = self.createFronts(r)
