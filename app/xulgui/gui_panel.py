...
class IndividualPanel(wx.Panel):
    ...
    def __init__(self, parent, id, widget_style, grid, ui_data, size = None):
        wx.Panel.__init__(self, parent = parent, size = (300,300), style = wx.SIMPLE_BORDER)
...
#------------------------------------------#
    def addWidget(self, widget):
        ...
        elif widget[key] == 'textbox':
            _widget = self.widgets['textbox'](self, -1, widget['value'])

        elif widget[key] == 'button':
            _widget = self.widgets['button'](self, -1, widget['value'])

        elif widget[key] == 'slider':
            _widget = self.widgets['slider'](self, -1, size = eval(widget['size']))

        elif widget[key] == 'combobox':
            _widget = self.widgets['combobox'](self, -1, size = eval(widget['size']), choices = eval(widget['value']))
