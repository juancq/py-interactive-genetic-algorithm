def f1(*stats):
    ...
    for k in range(0,len(dims)/2):
        roomDesc.append(dims[2*k])
        roomSizes.append(dims[2*k+1])
    ...
    if len(revisedArea) == 1:                      
        for j in range(0,len(roomSizes)):
            if roomDesc[j] != 'S':
    ...
    for j in range(0,len(roomDesc)):
        roomSizes.insert(2*j,roomDesc[j])

    roomSizes.append(len(revisedArea))
    return roomSizes
